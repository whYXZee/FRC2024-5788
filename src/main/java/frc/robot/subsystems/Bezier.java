...
// Bindings of the PathFlow Project by Krishna Shah (@DragonflyRobotics) and Jeevan Adhya Vinoth Babu (@InvisibleTiger) 
// https://github.com/DragonflyRobotics/PathFlow
    ...
    return t -> {
      ...
      if (t <= 0.0) {
        ...
        for (int i = 0; i < points.length; i++) {
          ...
          pose = new Pose2d(pose.getX() + X, pose.getY() + Y,
              Rotation2d.fromDegrees(pose.getRotation().getDegrees() + theta));
        }
        pose = new Pose2d(pose.getTranslation(),
            Rotation2d.fromRadians(MathUtil.angleModulus(pose.getRotation().getRadians())));
        return pose;
      }
    };
  }
